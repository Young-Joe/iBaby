...
import com.bjgoodwill.customlibrary.R;
...
public class SystemBarHelper {
     ...
     *
     * @param activity       Activity对象
     * @param drawableRes    状态栏drawable
     * @param alpha          透明栏透明度[0.0-1.0]
     */
    public static void tintStatusBarDrawable(Activity activity, @DrawableRes int drawableRes, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        tintStatusBarDrawable(activity.getWindow(), drawableRes, alpha);
    }
     ...
     * @param alpha          透明栏透明度[0.0-1.0]
     */
    public static void tintStatusBarDrawable(Window window, @DrawableRes int drawableRes, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        ...
        setStatusBarDrawable(decorView, drawableRes, true, false);
        setTranslucentView(decorView, alpha);
    }
    ...
    /** 创建假的状态栏View */
    private static void setStatusBarDrawable(ViewGroup container, @DrawableRes int drawableRes, boolean visible, boolean addToFirst) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            View statusBarView = container.findViewById(R.id.statusbar_view);
            if (statusBarView == null) {
                statusBarView = new View(container.getContext());
